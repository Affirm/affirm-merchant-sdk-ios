...
} AffirmExtendedProtocol;

static AffirmExtendedProtocol *allExtendedProtocols = NULL;
...
Method *_protocol_extension_create_merged(Method *existMethods, unsigned existMethodCount, Method *appendingMethods, unsigned appendingMethodCount)
{
...
void _protocol_extension_merge(AffirmExtendedProtocol *extendedProtocol, Class containerClass)
{
    // Instance methods
    ...
    Method *mergedInstanceMethods = _protocol_extension_create_merged(extendedProtocol->instanceMethods,
    ...
    Method *mergedClassMethods = _protocol_extension_create_merged(extendedProtocol->classMethods,
...
static void _protocol_extension_inject_class(Class targetClass, AffirmExtendedProtocol extendedProtocol)
{
    ...
    @autoreleasepool {
        for (unsigned protocolIndex = 0; protocolIndex < extendedProtcolCount; ++protocolIndex) {
            ...
            for (unsigned classIndex = 0; classIndex < classCount; ++classIndex) {
                ...
                _protocol_extension_inject_class(class, extendedProtcol);
            }
        }
    }
    pthread_mutex_unlock(&protocolsLoadingLock);
